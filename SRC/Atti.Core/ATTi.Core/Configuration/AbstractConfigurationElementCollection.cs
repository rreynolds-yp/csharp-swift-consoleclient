namespace ATTi.Core.Configuration
{
		...
		where TElement : ConfigurationElement, new()
	{
		...
		public IEnumerable<TElement> StrongTypeEnumerable
		{
			get { return new ElementEnumerable(this); }
		}

		private class ElementEnumerable : IEnumerable<TElement>
		{
			AbstractConfigurationElementCollection<TElement, TKey> _target;

			internal ElementEnumerable(AbstractConfigurationElementCollection<TElement, TKey> target)
			{
				_target = target;
			}
			...
			{
				IEnumerator e = _target.GetEnumerator();
				while (e.MoveNext())
				{
					TElement elm = e.Current as TElement;
					yield return elm;
				}
			}
